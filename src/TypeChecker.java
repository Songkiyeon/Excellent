...
public class TypeChecker {
	...
	public boolean checkBodyError(Statement s , ArrayList<valueTuple> valueTable) {
		...
		if(s.getClass().getName() == "Assignment") {
			...
			if(!check_expression((((Assignment)s).source),NameToType(((Assignment)s).target.toString(),valueTable),valueTable)) {
				return false;
			}
			return true;
		}
...
//			System.out.println("PR");
			...
			for(int i=0;i<((Def)s).Parameter_type.size();i++) {
				if(!check_declared_color(((Def)s).Parameter_type.get(i))) {
					...
					localTempTable.add(
						new valueTuple(
							((Def)s).Parameter_type.get(i),
							((Def)s).Parameter_name.get(i),
							colorTable
						));
				}
			}
		...
		if(e.getClass().getName().equals("Binary")) {//해결 인줄알았는데 미해결
			...
			if(((Binary)e).op.BooleanOp()) {
				...
				if(((Binary)e).op.val=="==" ||((Binary)e).op.val=="!=") {//이거 처리해야함?
					...
					if(check_expression(((Binary)e).term1, "int",valueTable) ||check_expression(((Binary)e).term1, "float",valueTable)) {
						if(check_expression(((Binary)e).term2, "int",valueTable) ||check_expression(((Binary)e).term2, "float",valueTable)) {
							if(type.equals("")) return true;
							if(type.equals("bool")) {
								return true;
							} else {
...
//			System.out.println("here : "+type);
			...
			errorMessage = "Different Type operation => " + ((Binary)e).term1.toString() + " " + ((Binary)e).op.toString()+ " " + ((Binary)e).term2.toString();
			return false;
//			return true;
